...
export default function Layout({ children, location, customLinks }) {
  return (
    <StaticQuery
      query={query}
      render={render(children, location, customLinks)}
    />
  );
}

const render =
  (children, location, customLinks = []) =>
  (data) => {
    ...
    const contextualLinks = (
      <ContextualLinks key={uuidv4()} links={contextual} />
    );
    return (
      <>
        <Seo />
